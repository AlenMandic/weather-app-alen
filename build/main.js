...
//setting default app initilization
let latitude = 40.71;
let longitude = -74.00;
let city_info = null;
...
let introduction_info = "New York, United States";

// default city information(New York)
...
current_elevation.textContent = "Elevation: 10m";
current_population.textContent = "Population: 8.5M";
...
weathercode_icon.setAttribute("alt", "Image of current weather condition");
...
current_windspeed_icon.setAttribute("alt", "Icon of current wind speed based on the beaufort wind scale.");
...
today_icon.setAttribute("alt", "Image of weather type for today.");
...
today_windspeed_icon.setAttribute("alt", "Icon of today's wind speed based on the beaufort wind scale.");
...
tomorow_icon.setAttribute("alt", "Image of expected weather type for tomorow.");
...
tomorow_windspeed_icon.setAttribute("alt", "Icon of tomorow's wind speed based on the beaufort wind scale.");
...
last_icon.setAttribute("alt", "Image of expected weather type for 2 days ahead.");
...
twodays_windspeed_icon.setAttribute("alt", "Icon of wind speed in 2 days time based on the beaufort wind scale.");
...
async function getWeather() {
  try {
    ...
    function displayHour(hourIndex, hourDiv) {
      ...
      hour_weathercode.setAttribute("alt", "Icon for expected hourly weather type.")
    ...
    // check if hourly data duplicates got created beacuse of the shift-tab keypress bug which i just couldn't fix.
...
function updateWeather() {
  if (!document.hidden) { //when document becomes visible, timer for updating starts.
    clearHourElements();
    getWeather();
    refreshTimeout = setTimeout(updateWeather, refreshInterval); // schedules recursive call for the next refresh (cycle)
  }
}
...
function getUserLocation() {
  //navigator success callback ( if user gives permission, and browser has navigator object )
